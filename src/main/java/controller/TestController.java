...
/*************************************************原生反序列化漏洞************************************************************************/
    ...
    /*******************************************************权限绕过测试*********************************************/
    @RequestMapping(value="/req/login",params = "test")
    //doGet,doPost
    public void req(HttpServletRequest request,HttpServletResponse response) throws Exception{
       ...
       // System.out.println(uri);
        if (uri.contains("/req/login")){
            //Javaweb response响应用法
            response.setContentType("text/html;charset=utf-8");
            PrintWriter out = response.getWriter();
            out.println("<br>getRequestURI：" + uri);
            out.println("<br>getRequestURL:" + url);
            out.println("<br>getContextPath:" + path);
            out.println("<br>getServletPath:" + servlet);
        }else {
            if (!uri.contains("/req/login"))
            response.setContentType("text/html;charset=utf-8");
            PrintWriter out = response.getWriter();
